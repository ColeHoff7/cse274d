...
template<class Node, class T>
Node* BinarySearchTree<Node,T>::getNode(T y) {
	Node *w = r, *z = nil;
	while (w != nil) {
		int comp = compare(y, w->x);
		if (comp < 0) {
			z = w;
			w = w->left;
		} else if (comp > 0) {
...
//go all the way to the bottom left, traverse up the tree until you find something larger than x, return
template<class Node, class T>
DLList<T> BinarySearchTree<Node, T>::getLE(T x){
	...
	while (u != nil) {
		...
		if(u->x <= x) list.add(u->x);
		else if(u->x > x) return list;
		prev = u;
		u = next;
	}
	return list;
...
template<class Node, class T>
void BinarySearchTree<Node, T>::preOrder(Node *u, int *n) {
	if (u == nil) return;
	u->pre_order = *n;
	*n +=1;
	preOrder(u->left, n);
...
template<class Node, class T>
void BinarySearchTree<Node, T>::postOrder(Node *u, int *n) {
	if (u == nil) return;
	postOrder(u->left, n);
	postOrder(u->right, n);
	u->pre_order = *n;
...
template<class Node, class T>
void BinarySearchTree<Node, T>::inOrder(Node *u, int *n) {
	if (u == nil) return;
	inOrder(u->left, n);
	u->pre_order = *n;
	*n += 1;
